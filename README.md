...
| ------------------ | ------------ | -------------------------------------------------- |
| `PRICE`            | Numérique    | **Cible** — prix de vente (87 500 → 665 000)       |
| `AREA`             | Numérique    | Surface totale en m² (33 → 324, moy. 102,6)        |
| `BEDROOMS`         | Numérique    | Nombre de chambres (1 → 6)                         |
| `BATHROOMS`        | Numérique    | Nombre de salles de bain (1 → 4)                   |
| `STORIES`          | Numérique    | Nombre d'étages (1 → 4)                            |
| `PARKING`          | Numérique    | Places de parking (0 → 3)                          |
...
### Comparaison baseline (test set — 218 observations)
...
Le nuage de points prédit / réel sur les 218 observations du test set montre des points très proches de la diagonale d'alignement parfait, sur l'ensemble de la plage de prix (100 000 – 615 000). Une légère dispersion est observable en milieu de gamme, ce qui est typique pour des données immobilières hétérogènes.
